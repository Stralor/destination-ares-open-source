...
namespace Kernys.Bson
{
	public class BSONValue
	{
		...
		protected static BSONValue convert(object obj) {
			...
			if (obj is float)
				return new BSONValue (obj as float);
		...
		*/
		...
		public float floatValue {
			get {
				...
				throw new Exception(string.Format("Original type is {0}. Cannot convert from {0} to float", mValueType));
			}
		}
		...
		public String stringValue {
			get {
				switch (mValueType) {
					...
					case ValueType.Float:
						return Convert.ToString (_float);
	...
	public class SimpleBSON
	{
		...
		private void encodeElement(MemoryStream ms, string name, BSONValue v) {
			switch (v.valueType) {
			case BSONValue.ValueType.Float:
				ms.WriteByte (0x01);
				encodeCString (ms, name);
				encodeFloat (ms, v.floatValue);
				return;
		...
		private void encodeFloat(MemoryStream ms, float v) {
